...
{
    public partial class LabReportWMP : PageBase
    {

        private static ILog log = LogManager.GetLogger(typeof(LabReportWMP));
        ...
        #region gvLabReportWMP Pre Render Event
        protected void gvLabReportWMP_OnPreRender(object sender, EventArgs e)
        {
            try
            {
                if (gvLabReportWMP.Rows.Count <= 0) return;
                gvLabReportWMP.UseAccessibleHeader = true;
                gvLabReportWMP.HeaderRow.TableSection = TableRowSection.TableHeader;
        ...
        #region gvLabReportWMP Row Command Event
        ...
        protected void imgbtnExcel_OnClick(object sender, EventArgs e)
        {
            try
            {
                ...
                string filename = "LabReportWMP_Report" + DateTime.Now.Date.ToString("dd-MM-yyyy") + ".xls";
                Response.AddHeader("content-disposition", "attachment;filename=" + filename);
                Response.Cache.SetCacheability(HttpCacheability.NoCache);
                StringWriter sw = new StringWriter();
                HtmlTextWriter hw = new HtmlTextWriter(sw);
                gvLabReportWMP.AllowPaging = false;
                gvLabReportWMP.GridLines = GridLines.Both;
                gvLabReportWMP.Columns[23].Visible = false;
                gvLabReportWMP.Columns[24].Visible = false;
                gvLabReportWMP.RenderControl(hw);
                string strSubTitle = "LAB REPORT WMP";
             ...
             "</div><br/><div align='center' style='font-family:verdana;font-size:11px'><span style='font-size:11px;color:Maroon;'>" + strSubTitle + "</ span><br/>" +
              "<br/>" + sw.ToString() + "<br/></div>";
                ...
                gvLabReportWMP.GridLines = GridLines.None;
                Response.Flush();
                Response.Clear();
                Response.End();
            }
        ...
        protected void imgbtnWord_OnClick(object sender, EventArgs e)
        {
            try
            {
                string filename = "LabReportWMP_Report" + DateTime.Now.Date.ToString("dd-MM-yyyy") + ".doc";
                Response.AddHeader("content-disposition", "attachment;filename=" + filename);
                Response.Charset = "";
                ...
                string content = "<div align='center' style='font-family:verdana;font-size:13px'><span style='font-size:16px;font-weight:bold;color:Maroon;'>LAB REPORT OF Wholed MILK POWDER TESTING</span>" +
            ...
            try
            {
                ...
                using (StringWriter sw = new StringWriter())
                {
                    using (HtmlTextWriter hw = new HtmlTextWriter(sw))
                    {
                        ...
                        PdfPTable pdfPTable = new PdfPTable(gvLabReportWMP.HeaderRow.Cells.Count);
                        ...
                        for (int i = 0; i < gvLabReportWMP.Rows.Count; i++)
                        {
                            if (gvLabReportWMP.Rows[i].RowType == DataControlRowType.DataRow)
                            {
                                for (int j = 0; j < gvLabReportWMP.Columns.Count; j++)
                                {
                                    string cellText = Server.HtmlDecode(gvLabReportWMP.Rows[i].Cells[j].Text);
                                    gvLabReportWMP.Columns[1].Visible = false;
                                    DateTime dDate;
                                    double dbvalue;
                                    int intvalue;
                        ...
                        Response.AddHeader("content-disposition", "attachment;" + "filename=LabReportWMP" + DateTime.Now.Date.ToString("dd-MM-yyyy") + ".pdf");
                        Response.Cache.SetCacheability(HttpCacheability.NoCache);
