...
{
    public partial class WTPData : System.Web.UI.Page
    {
        private static ILog log = LogManager.GetLogger(typeof(WTPData));
        ...
        public void RecordsBind()
        {
            try
            {
               ...
               CultureInfo.InvariantCulture);
                ...
                objResult = new MilkWeighmentBl().WTP_SelectAll(dtFromDateTime, dtToDateTime);
                gvWTP.DataSource = objResult.ResutlDs.Tables[0];
                gvWTP.DataBind();
                //gvTotalQty.DataSource = objResult.ResutlDs.Tables[1];
                //gvTotalQty.DataBind();

                if (gvWTP.Rows.Count > 1)
                {
        ...
        protected void imgbtnPDF_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string filename = "WTP_Report_" + DateTime.Now.Date.ToString("dd-MM-yyyy") + ".pdf";
                Response.AddHeader("content-disposition", "attachment;filename=" + filename);
                Response.Cache.SetCacheability(HttpCacheability.NoCache);
                StringWriter sw = new StringWriter();
                HtmlTextWriter hw = new HtmlTextWriter(sw);
                gvWTP.AllowPaging = false;
                gvWTP.GridLines = GridLines.Both;
                gvWTP.RenderControl(hw);
                string Date = DateTime.UtcNow.AddHours(5.5).ToString();
                string strSubTitle = "Water Treatment Plant Report</br> as on " + Date;
                ...
                gvWTP.GridLines = GridLines.None;
                Response.End();
            }
            catch (Exception ex)
            {
        ...
        protected void imgbtnExcel_Click(object sender, EventArgs e)
        {
            try
            {
                Response.AddHeader("content-disposition", "attachment;filename=WTP_Report.xls");
                Response.Charset = "";
