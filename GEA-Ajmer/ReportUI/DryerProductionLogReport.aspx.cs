...
{
    public partial class DryerProductionLogReport : System.Web.UI.Page
    {
        ...
        protected void imgbtnExcel_OnClick(object sender, EventArgs e)
        {
            try
            {
                ...
                string filename = "DryerProductionLogReport" + DateTime.Now.Date.ToString("dd-MM-yyyy") + ".xls";
                Response.AddHeader("content-disposition", "attachment;filename=" + filename);
                Response.Cache.SetCacheability(HttpCacheability.NoCache);

                StringWriter sw = new StringWriter();
                HtmlTextWriter hw = new HtmlTextWriter(sw);
                gvDryingProductionLog.AllowPaging = false;
                gvDryingProductionLog.GridLines = GridLines.Both;
                foreach (TableCell cell in gvDryingProductionLog.HeaderRow.Cells)
                {
                    cell.BackColor = gvDryingProductionLog.HeaderStyle.BackColor;
                }
                foreach (GridViewRow row in gvDryingProductionLog.Rows)
                {
                    ...
                    foreach (TableCell cell in row.Cells)
                    {
                        if (row.RowIndex % 2 == 0)
                        {
                            cell.BackColor = gvDryingProductionLog.AlternatingRowStyle.BackColor;
                        }
                        else
                        {
                            cell.BackColor = gvDryingProductionLog.RowStyle.BackColor;
                        }
                ...
                gvDryingProductionLog.RenderControl(hw);
                string strSubTitle = "DRYER PRODUCTION LOG REPORT";
