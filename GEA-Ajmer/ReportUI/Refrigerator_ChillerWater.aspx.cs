...
{
    public partial class Refrigerator_ChillerWater : System.Web.UI.Page
    {
        private static ILog log = LogManager.GetLogger(typeof(Refrigerator_ChillerWater));
        ...
        public void RecordsBind()
        {
            try
            {
               ...
               CultureInfo.InvariantCulture);
                ...
                objResult = new MilkWeighmentBl().Refrigerator_ChilledWater_SelectAll(dtFromDateTime, dtToDateTime);
                gvRefChilledWater.DataSource = objResult.ResutlDs.Tables[0];
                gvRefChilledWater.DataBind();
                //gvTotalQty.DataSource = objResult.ResutlDs.Tables[1];
                //gvTotalQty.DataBind();

                if (gvRefChilledWater.Rows.Count > 1)
                {
                ...
                else
                {
                    ...
                    divNo.Visible = true;
                    gvRefChilledWater.DataSource = null;
                    gvRefChilledWater.DataBind();
        ...
        protected void imgbtnPDF_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string filename = "Ref_ChilledWater_Report_" + DateTime.Now.Date.ToString("dd-MM-yyyy") + ".pdf";
                Response.AddHeader("content-disposition", "attachment;filename=" + filename);
                Response.Cache.SetCacheability(HttpCacheability.NoCache);
                StringWriter sw = new StringWriter();
                HtmlTextWriter hw = new HtmlTextWriter(sw);
                gvRefChilledWater.AllowPaging = false;
                gvRefChilledWater.GridLines = GridLines.Both;
                gvRefChilledWater.RenderControl(hw);
                string Date = DateTime.UtcNow.AddHours(5.5).ToString();
                string strSubTitle = "Refrigerator-Chilled Water Report</br> as on " + Date;
                ...
                gvRefChilledWater.GridLines = GridLines.None;
                Response.End();
            }
            catch (Exception ex)
            {
        ...
        protected void imgbtnExcel_Click(object sender, EventArgs e)
        {
            try
            {
                Response.AddHeader("content-disposition", "attachment;filename=Ref_ChilledWater_Report.xls");
                Response.Charset = "";
